...
| C5 | Custom categories (up to 3, free text) and music-link validation (F81) | Presets only | Add a "＋ Custom" chip (max 3 custom categories, ≤ 24 characters each). Validate music URLs against Spotify, Apple Music, YouTube and SoundCloud patterns before submitting. |
...
| D2 | Push tap routing for every type (F106) | Partial | Add routing for the moment types (03 §B6) and for `archive_warning` (opens the inbox Archived tab). Add a single `PushRoute` mapping table with tests. |
