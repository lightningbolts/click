...
- [ ] Event detail: People → Directory opens for every signed-in viewer; Interests and Mutuals show their per-person metrics before RSVP/check-in
- [ ] Event detail: Mutuals here uses the active sort with no duplicates under Everyone; RSVP/check-in only changes engagement CTAs
