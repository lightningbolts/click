...
- [ ] Home photo pile: availability pill + recap/insights/stats render outside piles (linear composables); one full-width roughly-square stack per pile section (~half screen height, 3 visible layers with vertical peek); drag tracks finger 1:1 with rubber-band past ~85%; omnidirectional swipe past threshold throws off with velocity; lower-left or opposite-last-throw recalls last dismissed card; tap jiggles then fans; below threshold springs back; stacked layers show scale/dim/elevation depth + ±4° rest tilt on all layers; collapse uses the same stagger in reverse; Reduce Motion replaces tilt/spring/stagger with a plain fade
