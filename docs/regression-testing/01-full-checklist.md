...
- [ ] Home photo pile: one full-width roughly-square stack per section (~half screen height); drag the top card and it tracks the finger 1:1; swipe up/right throws it off with swipe velocity; swipe left/down recalls the last dismissed card (grabbable mid-flight); below threshold springs back; stacked layers show scale/dim/elevation depth; tap fans the cluster out with a staggered deal; collapse uses the same stagger in reverse; Reduce Motion replaces tilt/spring/stagger with a plain fade
