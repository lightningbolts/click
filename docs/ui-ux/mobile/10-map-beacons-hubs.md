...
| Sheet dismiss | iOS body sheets default to **UIKit scroll-host** (`useUiKitScrollHost=true`, which-pin / view-event path): system grabber + scroll-edge expand/dismiss. Text-input sheets (global search, drop beacon, pickers) keep sticky field chrome + `sheetImePadding` and `ClickSheetDefaults.ContentTopPaddingUnderGrabber`. Do not use Compose fill-sheet surface-drag for forms (flicker). Android: Material grabber + nested-scroll dismiss via `SheetFingerDismissHost`. Hub↔Event swaps clear IME before remounting fields to avoid height thrash. |
