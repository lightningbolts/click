...
| Sheet dismiss | iOS Column/`sheetBodyScroll` sheets default to **UIKit scroll-host** (`useUiKitScrollHost=true`, which-pin / view-event path): system grabber + scroll-edge expand/dismiss. LazyColumn / HorizontalPager / sticky-IME sheets (search, profile tabs, member picker, availability) use **`useUiKitScrollHost=false`** + Compose fill + `sheetImePadding` + `ClickSheetDefaults.ContentTopPaddingUnderGrabber` (UIKit unbounded wrap → infinity height / empty lists / IME black gap). Android: Material grabber + nested-scroll dismiss via `SheetFingerDismissHost`. Hub↔Event swaps clear IME before remounting fields to avoid height thrash. |
