...
- **Two separate color systems.** UI **chrome** and generated **content** identity are independent and must not be re-coupled:
  - **Chrome accent ratio:** `ClickAccent` / `accentColor(AccentRole)` is **5/8 purple, 3/8 blue** (~62.5 / 37.5), inside the product 60/40–65/35 band. Owned by `ClickAccent.PURPLE_BUCKETS` / `TOTAL_BUCKETS`. Do not hardcode per-screen purple/blue mixes.
  - **Generated content palette:** `generateCardVisual(id)` draws from seven `CardHueFamily` buckets (purple, blue, teal, coral, gold, magenta, green) on its own weighted 16-slot table. Purple stays the heaviest bucket (5/16 ≈ 31%) so content reads on-brand, but two events never look alike. `CardVisual.hueFamily` exposes the choice; `purpleDominant` is kept only for map/pin parity with click-web.
- **Contrast safeguard (not a heuristic).** `ui/theme/Contrast.kt` provides `contrastRatio` and `scrimAlphaForContrast`, which search upward for the smallest scrim alpha that keeps white text at **4.5:1** over *every* gradient stop. `generateCardVisual` uses it for `contentScrim`, so bright gold/coral hues are as readable as deep purple. Always draw `contentScrim` behind text on a generated surface.
- **Generated card / beacon visuals:** paint them through the shared `ui/components/CardVisualSurface.kt` — `Modifier.cardVisualBackground(visual)` or `CardVisualHero(...)` — never a hand-rolled gradient. Every surface for the same entity (home pile, Events list, share pickers, chat beacon cards, profile Beacons tab, search rows, detail-sheet headers, map pin) seeds with the **raw entity id** (`beacon.id`), not a list-key prefix, so one item has one identity everywhere. Chrome, buttons, and plain list cards stay solid Functional Clarity (no gradients).
- **Decorative headers only.** `CardVisualHero` takes no title/subtitle: a detail sheet's structured section is the single source of title, schedule, and location. `chipLabel` (a short category tag) is the only text a hero may render.
- **Section labels:** one shared `SectionHeader` (bold title, optional caption, optional trailing action) for every list section — home pile rows, linear home, saved events, explore. Do not overlay a section label on top of the cards it labels.
- **Media source buttons:** `MediaSourceButton` (bordered icon-over-label tile) is the shared control for "Take photo" / "Photo library" in avatar onboarding and beacon drop. A bare `TextButton` next to body copy reads as highlighted text, not a button.
- **Single-line placeholders never wrap.** Pass plain hints via `ClickOutlinedTextField(placeholderText = "…")`, which caps them at one ellipsized line when `singleLine` is true. A wrapping placeholder grows the decoration box past the 56dp single-line height, which strands the vertically-centered caret in the middle of a double-height field. Custom `placeholder` slots are height-clamped to one line in single-line mode for the same reason.
