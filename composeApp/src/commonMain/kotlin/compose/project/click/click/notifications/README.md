...
The `notifications` package bridges **server-initiated push delivery** (Supabase Edge Function `send-push-notification`) with **client-side routing** into chats, hubs, and collaboration reveals. It handles FCM on Android, APNs on iOS, token registration lifecycle, and deep-link intent queues consumed by `App.kt`.
...
| **iOS** | APNs standard alerts | `standard` | Banner / sound / badge |
...
- Per-`tokenType` pending slots (FCM standard)
...
Server resolves `recipient_user_id` from chat membership for `chat_message`. `incoming_call` payloads are rejected.
...
| `NotificationRuntimeState` | In-memory message toggle mirror for foreground suppression |
...
| iOS `iOSApp.swift` | UNUserNotificationCenter |
