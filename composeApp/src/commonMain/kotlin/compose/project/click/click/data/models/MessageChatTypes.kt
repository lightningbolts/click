...
fun Message.parsedMediaMetadata(): MessageMediaMetadata? {
    ...
    val url =
        root["media_url"]?.jsonPrimitive?.contentOrNull?.takeIf { it.isNotBlank() }
            ?: root["mediaUrl"]?.jsonPrimitive?.contentOrNull?.takeIf { it.isNotBlank() }
    val dur =
        root["duration_seconds"]?.jsonPrimitive?.contentOrNull?.toIntOrNull()
...
/**
 * Private hub-media records keep their stable object path in message metadata. Unlike a signed
 * URL, this value is safe to persist and must be resolved through the hub gatekeeper per view.
 */
fun Message.hubMediaPathOrNull(): String? {
    val root = metadata as? JsonObject ?: return null
...
fun Message.hasLocalMediaUri(): Boolean = mediaUrlOrNull()?.startsWith("file://", ignoreCase = true) == true
...
fun replySnippetForMessage(
    message: Message,
    maxLen: Int = 140,
): String = replySnippetForMetadata(message.previewLabel(), maxLen)
