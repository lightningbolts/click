...
internal fun MapViewModel.toggleBeaconCheckInServerAuthoritativeImpl(beaconId: String) {
    ...
    if (currentlyCheckedIn) {
        viewModelScope.launch {
            try {
                    ...
                    onFailure = { error ->
                        ...
                        _engagementSnackbar.value =
                            beaconCheckInFailureMessage(
                                ...
                                fallback = "Couldn't check out. Please try again.",
                            )
                    },
                )
                ...
                _engagementSnackbar.value = "Couldn't check out. Please try again."
            } finally {
                _beaconCheckInPendingIds.update { it - id }
            }
        }
    ...
    viewModelScope.launch {
        try {
            ...
            _engagementSnackbar.value = "Couldn't check in. Please try again."
        } finally {
            _beaconCheckInPendingIds.update { it - id }
        }
    }
