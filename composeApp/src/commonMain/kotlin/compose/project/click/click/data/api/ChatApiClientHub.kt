 ...
 */
internal suspend fun ChatApiClient.searchConversationsImpl(
    query: String,
    authToken: String,
): Result<List<ChatApiClient.ConversationSearchHitDto>> =
    try {
        val q = query.trim()
        if (q.length < 2) return Result.success(emptyList())

        suspend fun getOnce(bearer: String): Pair<Int, Result<List<ChatApiClient.ConversationSearchHitDto>>> {
            val response =
