...
internal fun ConnectionViewModel.handleInstantProximityOutcome(
    outcome: BindProximityHandshakeOutcome,
...
internal fun ConnectionViewModel.handleAwaitingHostSelection(
    ...
    val groupIds =
        awaiting.groupCliqueCandidateMemberIds
            ?.distinct()
            ?.sorted()
            .orEmpty()
    _connectionState.value =
    ...
    proximityRecoveryJob =
        viewModelScope.launch {
            repeat(ConnectionViewModel.PENDING_MATCH_RECOVERY_ATTEMPTS) {
                ...
                if (_connectionState.value !is ConnectionState.ProximityHandshakePendingMatch) return@launch
                when (recovered) {
...
internal fun ConnectionViewModel.shouldBlockForRateLimit(
    users: List<User>,
    aggregateEncounterLogged: Boolean,
): Boolean {
...
internal fun ConnectionViewModel.startTapProximityHandshakeImpl(
    ...
    val previousHandshake = proximityHandshakeJob
    proximityHandshakeJob =
        viewModelScope.launch {
            previousHandshake?.join()
            try {
                ...
                val tokenStorage = repository.tokenStorage
                ...
                coroutineScope {
                    ...
                    val listenResult: ProximityHandshakeListenResult =
                        if (simulatorMock) {
                            delay(2_000L)
                    ...
                    if (bindResult.isSuccess) {
                        when (val handshake = bindResult.getOrNull()!!) {
                            is BindProximityHandshakeResult.PendingServerMatch -> {
                                ...
                                _connectionState.value =
                                    ConnectionState.ProximityHandshakePendingMatch(
                        ...
                        if (e.isRetryableForProximityBind()) {
                            ...
                            _connectionState.value =
                                ConnectionState.ProximityCapturedOfflineSyncing(
...
internal fun ConnectionViewModel.tryFlushPendingProximityHandshakesImpl(
    jwt: String,
    currentUserId: String,
) {
    if (jwt.isBlank()) {
