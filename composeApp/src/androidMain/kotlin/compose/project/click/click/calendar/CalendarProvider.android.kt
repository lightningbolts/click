...
actual class CalendarProvider {
    ...
    actual fun getAccessStatus(): CalendarAccessStatus =
        when (ContextCompat.checkSelfPermission(context, Manifest.permission.READ_CALENDAR)) {
            ...
            context.contentResolver
                .query(
                    ...
                    while (cursor.moveToNext()) {
                        ...
                        val availability =
                            if (availabilityIdx >= 0) cursor.getInt(availabilityIdx) else CalendarContract.Events.AVAILABILITY_BUSY
    ...
    actual suspend fun listCalendars(): List<DeviceCalendar> =
        withContext(Dispatchers.IO) {
            if (getAccessStatus() != CalendarAccessStatus.Granted) return@withContext emptyList()
            runCatching { queryDeviceCalendars(context) }.getOrDefault(emptyList())
        }
}

private var calendarContext: Context? = null

internal fun CalendarProvider.queryDeviceCalendars(context: Context): List<DeviceCalendar> {
    val projection =
        arrayOf(
    ...
    context.contentResolver
        .query(
            ...
            while (cursor.moveToNext()) {
                ...
                calendars += DeviceCalendar(id = cursor.getLong(0), name = name, accountName = cursor.getString(2)?.trim())
            }
        }
    return calendars
}
