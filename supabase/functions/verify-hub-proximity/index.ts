...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: venue, error } = await supabase
      .from("hub_venues")
      .select("id, name, geofence_lat, geofence_long, radius_meters, creator_id, expires_at, event_beacon_id")
      .eq("id", hubId)
      .maybeSingle();
    ...
    const eventBeaconId =
      typeof venue.event_beacon_id === "string" ? venue.event_beacon_id.trim() : "";

    if (eventBeaconId) {
      const isHost =
        typeof venue.creator_id === "string" && venue.creator_id === authData.user.id;
      if (!isHost) {
        const { data: checkIn } = await supabase
          ...
          .maybeSingle();
        const active = checkIn != null && checkIn.checked_out_at == null;
        if (!active) {
          return jsonResponse(
            {
    ...
    return jsonResponse({
      ...
      event_beacon_id: eventBeaconId || null,
    }, 200);
  } catch (e) {
